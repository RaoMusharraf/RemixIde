...
contract Marketplace is ReentrancyGuard , Ownable{
    ...
    */
    ...
    function IncreaseListTime(address _mintContract,uint256 _tokenId,uint256 _totalListTime) external {
        require(_idToNFT[_mintContract][_tokenId].listed,"Not Listed In Marketplace!");
        require(!NftAuction[_mintContract][_tokenId].isActive,"Already Listed In Auction!");
        require(_totalListTime >= 1, "Bid Time Must Be One Hour!");
        _idToNFT[_mintContract][_tokenId].start = block.timestamp;
        _idToNFT[_mintContract][_tokenId].end = block.timestamp + (3600 * _totalListTime);
    }
    // ============ BuyNFTs FUNCTIONS ============
    /*
    ...
    */
    function buyNft(uint listIndex,uint256 price,uint typ) public payable nonReentrant {
        ...
        uint nftQuantity = IConnected(listCount[listIndex].contractAddress).balanceOf(msg.sender);
        if(nftQuantity == 0){
            IConnected(listCount[listIndex].contractAddress).setApprove(msg.sender);
        }
    ...
    */
    ...
    function ClaimNFT(uint _auctionListCount,uint typ) external payable {
        require(block.timestamp > NftAuction[auctionListCount[_auctionListCount].contractAddress][auctionListCount[_auctionListCount].tokenId].endTime,"Bidding END!");
        uint nftQuantity = IConnected(listCount[_auctionListCount].contractAddress).balanceOf(msg.sender);
        if(nftQuantity == 0){
            IConnected(listCount[_auctionListCount].contractAddress).setApprove(msg.sender);
        }
        address owner;
        uint price;
    ...
    */
    ...
    function getBalance(address _mint) external view returns(uint){
        uint nftQuantity = IConnected(_mint).balanceOf(msg.sender);
        return nftQuantity;
    }
}