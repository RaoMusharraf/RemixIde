...
contract TokenStaking is Ownable{
    ...
    */
    function DepositTokens(address to,uint256 _amount,uint256 StakeMonth,uint256 EarnPersentage) public {
        if(Details[to].check){
            require(((Details[to].StakeMonth*60) + Details[to].stakeTime) > block.timestamp,"Your Time Period Complete.");
    ...
    */
    function WithdrawTokens(address to) public {
        ...
        require(((Details[to].StakeMonth*60) + Details[to].stakeTime) < block.timestamp,"Please Wait");
        uint InterestAmount;
        uint EarnToken;
        uint BurnToken;
