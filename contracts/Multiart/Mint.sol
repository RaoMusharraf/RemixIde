...
contract Minting is ERC721URIStorage, Ownable {
    ...
    constructor(address _adminAddress,address _mintPriceReceiver, address _transferFeeReceiver,address _tokenAddress,address _royaltyAddress)
        ERC721("Multiart", "MAT")
    {
    ...
    function safeMint(uint256 feePercentage,string memory uri,uint StartTime,uint EndTime,string memory NFT_doc,uint mintQuantity,uint TotalQuantity,address artist,uint perNFTPrice,uint typ,string memory collectionId) public payable {
        ...
        uint256[] memory mintedTokens = new uint256[](mintQuantity);
        ...
        for(uint i =1 ; i <= mintQuantity ; i++){
            ...
            mintedTokens[i-1] = _tokenIdCounter.current();
        }
        ...
        if(typ == 1){
            ...
            payable(mintPriceReceiver).transfer(((perNFTPrice*mintQuantity) - calculatedFeePrice));
            payable(transferFeeReceiver).transfer(calculatedFeePrice);
        }
        else if(typ == 2){
            require(IERC20(tokenAddress).balanceOf(msg.sender) >= (perNFTPrice*mintQuantity), "Invalid Price");
            IERC20(tokenAddress).safeTransferFrom(msg.sender,mintPriceReceiver,((perNFTPrice*mintQuantity) - calculatedFeePrice));
        }
        else{
            revert("Please Enter the correct payment Type");
        }
        emit SafeMinting(mintedTokens,msg.sender,block.timestamp);
    }
