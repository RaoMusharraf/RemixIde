...
contract Marketplace is ReentrancyGuard , Ownable{
    ...
    event Fee(address ArtistAddress,uint ArtistFee,address RoyalityAddress,uint RoyalityFee);
    ...
    */
    function buyNft(uint listIndex,uint256 price, uint _royltyPercentage, address _royalityAddress) public payable nonReentrant {
        ...
        uint sellerAmount = _idToNFT[listCount[listIndex].contractAddress][listCount[listIndex].tokenId].price - (royaltyAmount + artistFee + buyerFeeCul + sellerFeeCul);
    ...
    */
    function ClaimNFT(uint _auctionListCount, uint _royltyPercentage, address _royalityAddress) external payable {
        ...
        uint256 sellerAmount = selectedUser.price - (royaltyAmount + artistAmount);
